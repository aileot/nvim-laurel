...
(lambda let! [scope ...]
  ...
  (if (hidden-in-compile-time? scope)
      ...
      (case (case scope
              ...
              :b (values 3 `vim.api.nvim_buf_set_var `vim.api.nvim_buf_set_var)
              :w (values 3 `vim.api.nvim_win_set_var `vim.api.nvim_win_set_var)
...
(fn deprecate-nvim-laurel [call]
  (fn [...]
    (deprecate "the module `nvim-laurel.macros`" ;
               "new module `laurel.macros`" ;
               :v0.8.0 (call ...))))

;; Export ///1

;; Note: The macros without `deprecate-nvim-laurel` fails to expand with error
;; in some formats.
{:map! (deprecate-nvim-laurel map!)
 :unmap! (deprecate-nvim-laurel unmap!)
 :<Cmd> (deprecate-nvim-laurel <Cmd>)
 :<C-u> (deprecate-nvim-laurel <C-u>)
 :augroup! (deprecate-nvim-laurel augroup!)
 :autocmd! autocmd!
 :au! autocmd!
 :set! set!
 :setlocal! setlocal!
 ...
 :v! v!
 :env! env!
 :command! (deprecate-nvim-laurel command!)
 :feedkeys! (deprecate-nvim-laurel feedkeys!)
 :highlight! (deprecate-nvim-laurel highlight!)
 :hi! (deprecate-nvim-laurel highlight!)
 :let! (deprecate-nvim-laurel let!)}
